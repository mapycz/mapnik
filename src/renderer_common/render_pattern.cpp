...
using image_type = common_pattern_process_visitor::image_type;

common_pattern_process_visitor::common_pattern_process_visitor(
...
image_type common_pattern_process_visitor::operator() (marker_null const &) const
{
    throw std::runtime_error("This should not have been reached.");
}

template <typename Marker>
image_type common_pattern_process_visitor::operator() (Marker const & marker) const
{
...
template
image_type common_pattern_process_visitor::operator() (
    marker_rgba8 const & marker) const;

template
image_type common_pattern_process_visitor::operator() (
    marker_svg const & marker) const;
...
agg::trans_affine common_pattern_process_visitor::transform(box2d<double> & bbox) const
{
...
image_type common_pattern_process_visitor::render_pattern(
    ...
    agg::trans_affine tr) const
{
    ...
    image_type image(bbox.width() + spacing_x_, bbox.height() + spacing_y_);
...
image_type common_pattern_process_visitor::render_pattern_alternating(
    marker_svg const & marker,
    box2d<double> const & bbox,
    agg::trans_affine tr) const
{

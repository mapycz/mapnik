...
private:
    void prepare_layer(
        std::reference_wrapper<const attributes> vars,
        std::reference_wrapper<const Map> map,
        eAttributeCollectionPolicy collection_policy,
        double scale,
        double scale_denom,
        unsigned width,
        unsigned height,
        ...
        int buffer_size)
    {
        ...
        processor_context_ptr current_ctx = ds->get_context(ctx_map_);
        proj_transform prj_trans(proj0_, proj1_);
        ...
        boost::optional<box2d<double>> const& maximum_extent = map.get().maximum_extent();
        if (maximum_extent)
        {
        ...
        if (early_return)
        {
            ...
            {
                boost::optional<feature_type_style const&> style=map.get().find_style(style_name);
                if (!style)
                {
                    continue;
                }
        ...
        std::vector<rule_cache> & rule_caches = rule_caches_;
        attribute_collector collector(names_);
        ...
        q.set_variables(vars.get());

        if (collection_policy == COLLECT_ALL)
        {
...
template <typename Processor>
void feature_style_processor<Processor>::render_submaterials(

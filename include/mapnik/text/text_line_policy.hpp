...
struct text_line_policy
{
    ...
    text_line_policy(
        ...
        : path_(path),
          ...
          label_position_tolerance_(lg.get_text_props().label_position_tolerance)
    {
    }
    ...
    const double label_position_tolerance_;
};
...
struct marker_line_policy
{
    ...
    bool move(double distance)
    {
        return path_.move(distance) && (path_.linear_position() + layout_width_ / 2.0) < path_.length();
    }
